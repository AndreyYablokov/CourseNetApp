...
@log
def create_user_message(account_name='Guest'):
...
def create_exit_message(account_name='Guest'):
    ...
    logger.debug(f'Создано сообщение о выходе пользователя {account_name}')
    return message


@log
def receive_message(socket, account_name):
    while True:
        try:
...
@log
def user_interactive(socket, account_name):
    while True:
        command = input('Введите команду: ')
        if command == 'message':
            message = create_user_message(account_name)
            try:
            ...
            message = create_exit_message(account_name)
            try:
                send_message(socket, message)
            except Exception:
                logger.critical('Потеряно соединение с сервером')
...
def main():
    ...
    try:
        ...
        presence_message = create_presence_message(client_name)
        ...
        receiver_thread = threading.Thread(target=receive_message, args=(client_socket, client_name))
        receiver_thread.daemon = True
        receiver_thread.start()

        user_interface_thread = threading.Thread(target=user_interactive, args=(client_socket, client_name))
